...
def split_audio(input_file, folder_name, date_prefix, segment_time=420):
...
def process_single_file(seg_path, current_idx, total_files):
  """ประมวลผลไฟล์ย่อยแต่ละไฟล์ แปลและสร้าง TTS"""
  print("==================================================")
  print(
      f"🔄 กำลังประมวลผลไฟล์ [{current_idx}/{total_files}]:"
...
def concatenate_audio(tts_files, output_filename, folder_name):
  """นำไฟล์ mp3 ที่อ่านข่าวไทยทั้งหมดมาต่อกันเป็นไฟล์เดียว (แก้ไขปัญหา Path)"""
  if not tts_files:
  ...
  # 1. กำหนดชื่อไฟล์รายการเฉพาะกิจ
  list_filename = "concat_list.txt"
  list_file_path = os.path.join(folder_name, list_filename)

  # 2. เขียนชื่อไฟล์เสียงโดยใช้ basename (เนื่องจากเราจะใช้ cwd ทำงานในโฟลเดอร์นั้น)
  with open(list_file_path, "w", encoding="utf-8") as f:
    for audio_file in tts_files:
      base_name = os.path.basename(audio_file)
      f.write(f"file '{base_name}'\n")

  # 3. ดึงเฉพาะชื่อไฟล์ output เมื่อรัน ffmpeg ใน cwd
  base_output_name = os.path.basename(output_filename)
  ...
  cmd = [
      ...
      "0",
      "-i",
      list_filename,  # อ้างอิงชื่อไฟล์ตรงๆ ใน cwd
      "-c",
      "copy",
      base_output_name,  # อ้างอิงชื่อไฟล์ตรงๆ ใน cwd
  ]

  # สั่งรันคำสั่งโดยให้ FFmpeg ไปทำงานข้างใน folder_name
  ...
  # 4. ลบไฟล์รายการ text ชั่วคราวทิ้ง
...
if __name__ == "__main__":
  ...
  # สร้างชื่อโฟลเดอร์จาก [ชื่อ Workflow]_[เวลา-นาที]
  ...
  if success:
    ...
    segment_files = split_audio(
        main_file, folder_name, date_str, SEGMENT_DURATION
    )
    ...
    for idx, seg in enumerate(segment_files, start=1):
      ...
      # พัก 5 วินาที ลดความเสี่ยงการติด Rate Limit API
      time.sleep(5)

    # รวมไฟล์เสียงพากย์ไทยทั้งหมดเป็นไฟล์เดียว
